...
4.  Пробовал разные размеры `beam_size`. Не пробовал размер < 100, сразу начинал со 100. Проверил только, что качетсво при beam_size > 100 такое же, как на 100 (на инференс на всякий случай поставил 200, хуже ведь не будет).
5.  Прикрутил LM-ку вот отсюда: http://www.openslr.org/11. Вот только либо она не работает, либо я чего-то не понял.
6.  По причине медленного обучения переключился на fp16. Кажется качество от этого не падает, по крайней мере на первых k эпохах лоссы одинаково сходятся.
...
Таргет: <img src='images/ex_targ.jpeg' width='400'>
Предикт: <img src='images/ex_pred.jpeg' width='400'>
